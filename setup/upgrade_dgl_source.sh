...
echo "🔧 RFdiffusion DGL Source Upgrade Script"
echo "========================================="

# Check if we're in a conda environment
if [[ -z "${CONDA_DEFAULT_ENV}" ]]; then
    echo "❌ Error: Please activate a conda environment first"
...
echo "🗑️ Uninstalling current DGL..."
...
DGL_VERSION="v2.4.0"
echo "🔄 Checking out DGL $DGL_VERSION..."
...
if [[ "$PYTORCH_VERSION" =~ ^2\.7 ]]; then
    echo "🎯 PyTorch 2.7 detected - using CUDA 12.8 optimizations"
    CUDA_TOOLKIT_VERSION="12.8"
elif [[ "$PYTORCH_VERSION" =~ ^2\.[4-6] ]]; then
    echo "🎯 PyTorch 2.4-2.6 detected - using CUDA 12.4 optimizations"
    CUDA_TOOLKIT_VERSION="12.4"
else
    echo "🎯 PyTorch 2.3 or older detected - using CUDA 12.1 optimizations"
...
if [[ "$PYTORCH_VERSION" =~ ^2\.7 ]]; then
    echo "🔧 Setting PyTorch 2.7 + CUDA 12.8 optimizations..."
    export USE_CUDA=ON
...
print(f'✅ Basic DGL graph creation: OK')
...
try:
    ...
    print(f'⚠️ RFdiffusion test failed: {e}')
"
